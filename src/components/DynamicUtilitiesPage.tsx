...
const DynamicUtilitiesPage: React.FC = () => {
  ...
  const fetchWeatherData = async (location: LocationInfo): Promise<WeatherData> => {
    try {
      ...
      const searchUrl = `https://www.googleapis.com/customsearch/v1?key=REDACTED_GOOGLE_KEY&cx=017576662512468239146:omuauf_lfve&q=${encodeURIComponent(searchQuery)}`;
